...
conda-forge does not build from it. Its version and source SHA-256 follow
each release automatically: `release.yml`'s `release-metadata` job runs
`packaging/release.py sync-release-metadata`, which reads the sdist digest
from PyPI. On 2026-09-26 it names the published 1.5.1.0 PyPI archive. As of
2026-09-23 the feedstock and conda package were at 1.5.0.8. Their dependency pins
...
published package before describing an update as pending. The mirror's
version and source hash are refreshed by the release workflow; changing the
mirror does not trigger a conda-forge release.
