...
def canonical_sources() -> dict[str, object]:
    ...
    # A folded page is still user-facing even though it no longer owns a Home
    # tile.  Resolve those defaults too: otherwise a clean catalog build drops
    # Barcode QC, AnnData export and Explain-CV help while a long-lived GUI
    # process (which happened to import their modules) retains it.  Catalog
    # contents must not depend on import order.
    app_keys = list(dict.fromkeys(
        [str(row[0]) for row in APPS] + list(_FOLDED_DEFAULTS_MODULES)
    ))
