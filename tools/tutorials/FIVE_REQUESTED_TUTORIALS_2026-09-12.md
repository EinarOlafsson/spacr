...
| Tutorial | Verified work | Remaining limitation |
| --- | --- | --- |
...
Previous private model drafts and catalogs were archived, not deleted, at
`/mnt/firecuda2/Claude/toxoplasma_projects/tutorials/refresh_2026-09-09/models-before-refresh-cm83ngnt`.
The integration receipt is `refresh_2026-09-09/models-merge.json`; it records
the eight specific folders copied. No unrelated lesson was replaced.

Recorded helpers, scripts, all 26 model translations, web media and byte
identities are protected in Git. Narration and 4K masters remain in the full
local package, indexed by the committed manifest. Original recordings,
earlier candidates and source data remain intact.

Two full-browser failures exposed an interrupted-seek defect in the player:
replacing a video's source can cancel a seek without a `seeked` event. The
native `emptied` handler now clears that discarded resource's pending flag.
The real-browser test was observed failing before the fix and passing after
it, including synchronized playback in the next lesson. The completed package
subsequently passed the unchanged full-player checks; earlier failed logs and
