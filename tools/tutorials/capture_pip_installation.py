#!/usr/bin/env python3
"""Record real verification commands and the installed wheel's GUI.
...
skipped for this bounded launch, in a private profile on a private display.
"""
...
def installation(stage, folder):
    ...
    if (not result.get('accepted') or len(result.get('steps', [])) != 6
            or any(row.get('returncode') != 0 or not row.get('completed')
                   for row in result['steps'])):
        raise ValueError('The real installation must have six successful checks')
    return root, root / 'venv', result
...
def record(stage, root, venv, prior, capture):
    from PySide6.QtWidgets import QApplication
    from capture_diagnostics import PrivateDesktop

    capture.mkdir(parents=True)
    provenance = dict(completed_capture=False, module='pip_install',
                      ...
                      setup_skipped_explicitly=True, first_run_tour_marked_seen=True,
                      format_scope='Linux, fresh public PyPI wheel; not other platforms')
    ...
    if provenance['installed_identity']['version'] != prior['pypi']['version']:
    ...
    subprocess.run([str(venv / 'bin/python'), '-I', '-c',
        'from spacr.qt.first_run import mark_tour_seen; '
        'from spacr.qt.preferences import set_theme,set_font_scale,set_preload_policy; '
        'mark_tour_seen(); set_theme("dark"); set_font_scale(1.5); set_preload_policy("on_demand")'],
        cwd=root, check=True, timeout=90)
    provenance['recording_preferences'] = dict(theme='dark', font_scale=1.5, preload='on_demand')
    app = QApplication([])
...
def main():
    ...
    root, venv, prior = installation(stage, args.installation)
    ...
    env.update(SPACR_TUTORIAL_PRIVATE_DESKTOP='1', GIO_USE_VFS='local',
               ...
               VIRTUAL_ENV=str(venv), PATH=str(venv / 'bin') + os.pathsep + env['PATH'],
    ...
    return subprocess.run(['xvfb-run', '-a', '-s', '-screen 0 3840x2160x24',
        ...
        '--installation', str(root), '--capture-name', args.capture_name, '--inside'],
