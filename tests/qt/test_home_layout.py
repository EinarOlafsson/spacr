...
def test_every_app_is_on_exactly_one_subject_tab_and_one_home_band():
    ...
    # Thirty-eight, up from thirty-four: Pipeline Graph, Hit List,
    # Prediction Profiler and Methods & Results each arrive alpha, as Power /
    # Design, AnnData Export, Run Compare, Illumination, Barcode QC, Layer
    # Viewer, Graph Builder and Data Manager did before them. The count is
    # the user's list — it is how many of the apps in front of them carry
    # a "not signed off" colour, and it drops by one every time an app is
    # signed off.
    assert len(staged) == 38, (
        f"{len(staged)} apps staged, not 38 — if that is intended, say so "
