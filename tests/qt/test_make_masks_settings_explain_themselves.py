...
def _awaiting_translation(screen) -> set:
    """The panel captions the catalog lane has not reached yet.

    THE CATALOGS ARE NOT REGENERATED BY THE CHANGE THAT ADDS A CAPTION.
    ``spacr/qt/i18n_catalogs`` is 21 MB of generated rows in nine
    languages, built by ``tools/build_i18n_catalogs.py`` against a
    translation model; a feature branch that regenerated it would put a
    machine translation of the whole file into a diff about a settings
    card. So a new caption is LISTED here for the catalog lane instead of
    being silently allowed, and
    :func:`test_no_caption_waits_for_a_translation_it_already_has` fails
...
def test_every_help_string_on_the_panel_has_an_exact_catalog_row(screen):
    ...
    Except the captions :func:`_awaiting_translation` names, which are
    waiting on the catalog lane; its docstring says why they are not
    translated by the branch that wrote them.
    """
...
def test_no_caption_waits_for_a_translation_it_already_has(screen):
    """A caption the lane has translated leaves the waiting list.

    Without this the list above is a hole that stays open: a caption added
    to it once would go on being skipped after the catalogs learned it,
    and the next change to that caption would break its translation
    unnoticed.
    """
    from spacr.qt.i18n import has_translation
    from spacr.qt.i18n_catalogs import CATALOG_LANGUAGES
        ...
        source[:60]
        for source in _awaiting_translation(screen)
        if all(has_translation(source, code) for code in CATALOG_LANGUAGES)
    )
    assert not done, (
        "these captions are translated and must leave "
        "_awaiting_translation:\n" + "\n".join(done))
