...
def test_reviewed_scientific_terms_use_domain_context_not_false_friends():
    ...
    run_source = (
        "Review quality-control verdicts written by completed pipeline runs. "
        "The dashboard reads stored results without loading masks or "
        "recomputing scores. A card is marked out of date when its inputs "
        "are newer than the stored verdict. A missing card indicates that "
        "the corresponding check has not been run; it does not indicate a "
        "passing result."
    )
    run_text = french_catalog.UI[run_source]
    assert "contrôle qualité" in run_text and "piste" not in run_text.casefold()
