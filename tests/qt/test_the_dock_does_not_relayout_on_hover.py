...
def test_hover_follows_under_mouse_not_the_global_cursor(window, qapp):
    """`underMouse()` is asked first, because Wayland has no cursor to ask.

    THE BLINK SURVIVED A FIX FOR EXACTLY THIS. `sync_hover` read
    `QCursor.pos()`, and Wayland does not give a client the global pointer
    position -- Qt returns the last value it happened to see. A stale one
    puts the cursor over no row, the ink is cleared, the next event puts it
    back. Read off three consecutive frames of the maintainer's screen
    capture on 2026-09-03: icon blue, icon white, icon blue, with the
    pointer never leaving the row.

    Qt maintains `WA_UnderMouse` from its own Enter/Leave bookkeeping, which
    needs no global position and is correct on every platform.
    """
    from PySide6.QtCore import Qt as _Qt
    from PySide6.QtGui import QCursor

    dock = window._sidebar
    rows = [r for r in dock._items if not r.isHidden()][:3]
    target = rows[2]

    # The cursor says somewhere else entirely -- a stale Wayland answer.
    QCursor.setPos(dock.mapToGlobal(QPoint(dock.width() + 900, 900)))
    qapp.processEvents()
    for row in dock._items:
        row.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, False)
    target.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, True)

    inked = dock.sync_hover()
    assert inked == str(target.property("navKey")), (
        f"the ink went to {inked!r} rather than to the row that has the "
        "pointer -- the stale cursor won")
    assert target._hovered is True
    target.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, False)


def test_repeated_syncs_do_not_toggle_the_ink(window, qapp):
    """The blink, stated as the property it violates.

    Whatever arrives -- a row's Enter, a row's Leave, the dock's Leave, the
    same one twice -- the ink must land in the same place, because the
    pointer has not moved. Ten passes, one answer.
    """
    from PySide6.QtCore import Qt as _Qt

    dock = window._sidebar
    target = [r for r in dock._items if not r.isHidden()][1]
    for row in dock._items:
        row.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, False)
    target.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, True)

    seen = {dock.sync_hover() for _ in range(10)}
    target.setAttribute(_Qt.WidgetAttribute.WA_UnderMouse, False)
    assert seen == {str(target.property("navKey"))}, (
        f"ten identical syncs gave {seen} -- that is the blink")
