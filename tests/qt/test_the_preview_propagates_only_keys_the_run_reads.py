...
# This subset relation has caught controls that survived their pipeline
# settings being removed. It remains independent of the number of fields
# offered, so deleting a retired feature does not weaken the check.
...
def test_the_live_preview_sends_both_mean_bounds_for_every_area_filter(
        qtbot, qt_theme_applied):
    """The subset ratchet cannot detect a required control that is missing."""
    from spacr.qt.screens.app_screen import AppScreen
    ...
    roles = {key.removesuffix("_min_area") for key in propagated
             if key.endswith("_min_area")}
    assert roles, "no object area filters are propagated"
    for suffix in ("min_intensity", "max_intensity"):
        intensity_roles = {key.removesuffix(f"_{suffix}") for key in propagated
                           if key.endswith(f"_{suffix}")}
        assert intensity_roles == roles, (
            f"{suffix} does not cover the same objects as the area filters")
