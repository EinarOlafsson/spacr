...
def test_registry_has_complete_unique_exact_key_mapping():
    ...
    assert len(by_setting) == 96
    assert len({animation.slug for animation in animations}) == 78
...
def test_every_asset_is_square_animated_and_matches_manifest_hash():
    ...
    assert summary["setting_keys"] == 96
...
def test_an_exact_numbered_slot_mapping_takes_precedence(monkeypatch):
    ...
    exact = animation_for_setting("organelle_max_intensity")
