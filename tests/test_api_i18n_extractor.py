...
def test_public_docstrings_matches_reviewed_visible_coverage():
    ...
    #          10,242 -> 10,243 on 2026-09-08. ONE symbol, and it is a whole
    #          module: `spacr.qt.screens.settings_flow_index`, generated by
    #          `tools/settings_flow.py --rst` alongside the settings-flow
    #          page. It holds one frozenset of the keys that page drew a
    #          section for, and the settings panel imports it to decide
    #          whether a setting with no API anchor can be sent there
    #          instead of to the top of a 4,000-line module page (383).
    #
    #          Its module docstring is the single admitted symbol; the
    #          frozenset itself carries no PEP-258 docstring and is
    #          correctly invisible. It is NOT excluded from the surface,
    #          because its sibling `setting_api_targets` is not either --
    #          both are generated, both are imported at runtime, and a
    #          generated file that ships in the wheel is API.
    expected = 10_243
...
def test_public_docstrings_exclude_the_exact_non_rendered_autoapi_boundary():
    ...
    #   ---
    #   213
    # 10,455 -> 10,456 with settings_flow_index's module docstring, so the
    # BOUNDARY is unchanged at 213: the new symbol is rendered, not omitted.
    # Both numbers move together or the arithmetic silently reattributes a
    # new public symbol to the non-rendered pile.
