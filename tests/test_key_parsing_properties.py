...
Five more are confirmed, reproduced by an ``xfail(strict=True)`` test each,
...
@pytest.mark.xfail(strict=True, reason=(
    'BUG (spacr.selection, unfixed): object_keys is not injective. It joins '
    'plateID/rowID/columnID/fieldID/object_label with the schema separator '
    'and applies no guard to the components, so ("p_x", "r1", "c1", "f1", 1) '
    'and ("p", "x_r1", "c1", "f1", 1) both produce "p_x_r1_c1_f1_1". Two '
    'distinct objects become one key, and every view that resolves a key '
    'back to a row -- UMAP lasso, plate heatmap, annotation grid -- shows or '
    'annotates the wrong object. schema._check_plate refuses a separator in a '
    'plate id for exactly this reason; object_keys has no equivalent. The '
    'repair belongs in spacr/selection.py: reject a component containing the '
    'separator, or compose through schema instead of str.cat.'))
@given(label=object_labels)
