...
class TestSingleThreadedOpenmp:
    ...
    def test_recursive_decorator_calls_restore_the_outer_limit(
        self, monkeypatch, clamping_platform
    ):
        """Each call owns its state; ContextDecorator must not reuse it."""
        handle = self._fake_runtime(monkeypatch, maxima=10)
        monkeypatch.setattr(
            openmp_guard,
            "resident_openmp_runtimes",
        ...
        @openmp_guard.single_threaded_openmp("recursive training")
        def train(depth):
            if depth:
                return train(depth - 1)
            return handle("/a/libomp.dylib").value
