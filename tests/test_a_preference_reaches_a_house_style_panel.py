...
def test_the_palette_preference_does_not_reach_a_house_style_panel(prefs):
    """DELIBERATE, and recorded so it is not read as a gap.

    The house style fixes the colour VOCABULARY -- grey for everything the
    sentence is not about, one highlight for what it is -- and a seaborn
    categorical cycle laid over that is the rainbow the style exists to
    forbid. `rc_params` carries no palette key, so this holds by
    construction; the assertion is here to make the choice visible.
    """
