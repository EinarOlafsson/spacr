...
Status:    CLOSED, verified 2026-09-14. The table exists, it is GENERATED
           ...
           decides the answers at run time.
             * README.rst:73-113, between `.. spacr-hardware-begin` and
               `-end`, six rows plus the legend "supported (stable) /
               implemented (beta) / CPU support only".
             * Generated by packaging/generate_readme_visuals.py --
               HARDWARE_ROWS :576, HARDWARE_TASKS :594, `_capabilities_for()`
               :630, `_hardware_table()` :651, wired into main() at :1603.
               `_capabilities_for` constructs a real `Accelerator` and reads
               `accelerator.capabilities()` (spacr/accelerator.py:682), so the
               table cannot drift from the resolver: a backend that stops
               being supported changes the README on the next build.
             * spacr/doctor.py:1237 renders the SAME capabilities(), so what
               the README promises and what the doctor reports on the user's
               own machine come from one source.
             * tests/test_the_hardware_table_matches_the_resolver.py: 9 passed.
             * All nine locale READMEs carry an identical marker span (md5
               d0ae250d for README.rst and for every docs/i18n/readme/
               README.<locale>.rst), and tests/test_readme_presentation.py:153
               already carves the block out of the prose ceiling.
           Landed as d6d7397f6.
