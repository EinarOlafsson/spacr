...
| **403** | **The search that sets the settings: may it write into the form without a click?** You asked for "the automatic settings mode ... like live mode in make". Live preview updates continuously and WRITES NOTHING -- it is a view. The barcode search WRITES SETTINGS. Making the SEARCH re-run by itself is cheap (measured: 0.07 s over a 32 MB gzipped file of 2,000,000 reads, because the sample stops at 20,000) and contradicts nothing. Making the APPLY automatic is the thing item 399 refused on purpose, because a verdict moves from undecided to present to absent as the sample grows, so it would rewrite your form two or three times per run -- over values you typed. **Which did you mean?** | a decision |
