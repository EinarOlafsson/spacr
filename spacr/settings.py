...
tooltips = {
    ...
    "regression_panel_manifest": "(dict, str, or None) - Optional publication-panel specification applied after the regression CSV files have been written. Supply a mapping or a JSON/YAML path that explicitly declares each panel's result source, phenotype, gene/gRNA level and plot kind; spacr.ml._write_regression_panel_packages resolves those declarations against the run artifacts, and spacr.regression_panels.build_manifest_packages validates and writes the packages under publication_panels/. None skips publication-panel generation. This structured setting is available to scripts and saved settings but is intentionally not rendered as a one-line GUI field. Default None.",
