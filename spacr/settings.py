...
tooltips = {
    ...
    "cell_model_name": "(str) - Weights used to segment cells. Cellpose 4 provides one stock model, 'cpsam'; the alternative is a path to a CPSAM checkpoint produced by Train Cellpose, loaded as pretrained_model. Pre-SAM names ('cyto', 'cyto2', 'cyto3', 'nuclei') remain accepted for compatibility with older settings, but all resolve to cpsam because those historical models are unavailable in Cellpose 4. Of the three parameters that previously distinguished models, only diameter remains operational (eval rescales by 30/diameter); model_type and diam_mean are logged as 'not used in v4.0.1+' and omitted. Default 'cpsam'.",
    ...
    "guide_permutations": "(int) - Number of plate-blocked Freedman--Lane residual permutations used for empirical two-sided guide P values. For b permuted statistics at least as extreme as the observed statistic and B permutations, p=(b+1)/(B+1); the minimum attainable value is therefore 1/(B+1). Values of 1,000, 10,000, and 200,000 resolve minima of approximately 1e-3, 1e-4, and 5e-6, respectively. Increase the count when results accumulate at this resolution limit; runtime increases linearly. Default 200000.",
    ...
    "plot": "(bool) - Render and save quality-control figures during the pipeline, including channel montages, Cellpose mask overlays, filtration comparisons, and crop grids. Figure generation increases runtime and memory use, particularly for complete plates. test_mode enables this setting automatically. Default False.",
    ...
    "Signal_to_noise": "(int) - Multiplier on background that sets the signal threshold (background * Signal_to_noise) used for Cellpose normalization. Per channel, spaCR selects the first of the 98th, 99th, 99.9th, 99.99th and 99.999th percentiles above the threshold and rescales to it. Increasing the value reduces clipping and produces dimmer output; decreasing it increases faint-signal contrast but can saturate bright objects. If no percentile exceeds the threshold, the range collapses to the 2nd percentile, indicating that the value is too high. Ignored when percentiles is set. Default 10 (5 in check_cellpose_models).",
    ...
    "normalize_plots": "(bool) - Scale each displayed image to its own intensity range before rendering. This affects displayed figures but not measurements. Disable it when comparing brightness across images because independent scaling removes absolute intensity differences. Default True.",
    ...
    'organelle_diameter': "(float) - Deprecated. Expected organelle diameter in pixels. The Cellpose-SAM path used for organelles calls model.eval with diameter=None, and no classical method sizes its kernels from it, so changing this value has no effect on organelle masks. Bound object size with organelle_min_size / organelle_max_size instead. Default 30.",
    ...
    'organelle_max_size': "(int or None) - Upper area bound in square pixels applied to the final label image; objects above it are deleted rather than split. Use it to remove fused clumps, saturated debris and background regions merged by Otsu into a single component. Values below the largest valid organelle remove biological objects without warning. Default None (no limit).",
    'organelle_remove_border': "(bool) - Delete organelle labels touching any image edge during final post-processing so partially imaged objects do not bias area and intensity statistics. This also removes valid objects at the field boundary, with a larger effect for larger organelles. Default False.",
    ...
    'organelle_FT': "(float) - Cellpose flow_threshold: maximum error allowed between a candidate mask's flows and the network prediction. Lower values discard more irregular masks; higher values retain more irregular objects. Increase it when valid non-round organelles are being discarded. Default 0.4.",
    ...
    'organelle_min_object_area': "(int) - Absolute area threshold in square pixels for the split step. An object is split only when its area exceeds this value in addition to the median-based threshold, preventing division of small organelles into multiple labels. Default 100. Currently inactive because the organelle mask writer does not execute the merge/split stage.",
