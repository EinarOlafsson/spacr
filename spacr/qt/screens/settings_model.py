...
def _mapped_api_target(key: str) -> tuple[str, str]:
    """``(module_path_segment, anchor)`` for a setting, or ``("", "")``.

    Reads the table generated by ``tools/build_setting_consumer_map.py``. A
    missing table is not an error -- the caller falls back to the module link
    that shipped before instruction 336 -- so a checkout that has not run the
    generator still gets the old behaviour rather than no link.
    """
    if not key:
        return ("", "")
    try:
