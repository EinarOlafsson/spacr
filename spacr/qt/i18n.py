...
_ROWS: Dict[str, tuple[str, ...]] = {
    ...
    "What may leave this machine, and under whose name. With issue filing set to always, the default, a failed run files a redacted report on the public spaCR GitHub repository automatically, once per error, under the GitHub account signed in below. With ask, you see each report first and it is sent only when you press Send. With never, nothing is sent.": _row(
        ...
        "Qué puede salir de este equipo y a nombre de quién. Con always, la opción predeterminada para informar de errores, una ejecución fallida publica automáticamente un informe con los datos sensibles eliminados en el repositorio público de spaCR en GitHub, una vez por error, desde la cuenta de GitHub conectada abajo. Con ask, primero ve cada informe y solo se envía al pulsar Enviar. Con never, no se envía nada.",
        "哪些内容可以离开本机，以及以谁的名义发送。错误报告设为默认值 always 时，运行失败会自动在 spaCR 的公开 GitHub 仓库中提交已隐去敏感信息的报告，每个错误提交一次，使用下方登录的 GitHub 账户。设为 ask 时，您会先看到每份报告，只有点击发送后才会提交。设为 never 时，不发送任何内容。",
        "O que pode sair deste computador e em nome de quem. Com always, a opção padrão de relato de erros, uma execução com falha publica automaticamente um relatório com os dados sensíveis removidos no repositório público do spaCR no GitHub, uma vez por erro, pela conta do GitHub conectada abaixo. Com ask, você vê cada relatório primeiro, e ele só é enviado ao clicar em Enviar. Com never, nada é enviado.",
        "इस कंप्यूटर से क्या बाहर जा सकता है और किसके नाम से। त्रुटि रिपोर्टिंग के डिफ़ॉल्ट विकल्प always पर, असफल रन संवेदनशील जानकारी हटाकर सार्वजनिक spaCR GitHub रिपॉज़िटरी में अपने आप रिपोर्ट दर्ज करता है, हर त्रुटि के लिए एक बार, नीचे साइन इन किए गए GitHub खाते से। ask पर आप पहले हर रिपोर्ट देखते हैं और वह केवल भेजें दबाने पर भेजी जाती है। never पर कुछ नहीं भेजा जाता।",
        "이 컴퓨터에서 어떤 내용이 누구의 이름으로 전송될 수 있는지 정합니다. 오류 보고를 기본값인 always로 설정하면 실행 실패 시 민감한 정보를 가린 보고서가 공개 spaCR GitHub 저장소에 오류마다 한 번씩 자동으로 등록되며, 아래에 로그인한 GitHub 계정을 사용합니다. ask에서는 각 보고서를 먼저 확인하고 보내기를 눌러야 전송됩니다. never에서는 아무것도 전송되지 않습니다.",
        "Hvað má fara af þessari tölvu og í hvers nafni. Þegar villutilkynningar eru stilltar á always, sem er sjálfgefið, sendir keyrsla sem bregst sjálfkrafa skýrslu með viðkvæmum upplýsingum fjarlægðum í opinbera GitHub-kóðageymslu spaCR, einu sinni fyrir hverja villu, með GitHub-reikningnum sem er innskráður hér fyrir neðan. Með ask sérðu hverja skýrslu fyrst og hún er aðeins send þegar þú ýtir á Senda. Með never er ekkert sent.",
        "Ce qui peut quitter cet ordinateur et au nom de qui. Avec always, le réglage par défaut du signalement des erreurs, une exécution échouée dépose automatiquement un rapport dont les informations sensibles ont été masquées dans le dépôt public spaCR sur GitHub, une fois par erreur, au nom du compte GitHub connecté ci-dessous. Avec ask, vous voyez d'abord chaque rapport et il n'est envoyé que lorsque vous cliquez sur Envoyer. Avec never, rien n'est envoyé."),
    ...
    "An install is still running": _row("En installation pågår fortfarande", "Eine Installation läuft noch", "Todavía hay una instalación en curso", "仍有安装正在进行", "Ainda há uma instalação em andamento", "एक इंस्टॉलेशन अभी भी चल रहा है", "설치가 아직 진행 중입니다", "Uppsetning er enn í gangi", "Une installation est toujours en cours"),
    ...
    "Create a GitHub account": _row("Skapa ett GitHub-konto", "GitHub-Konto erstellen", "Crear una cuenta de GitHub", "创建 GitHub 账户", "Criar uma conta no GitHub", "GitHub खाता बनाएँ", "GitHub 계정 만들기", "Stofna GitHub-aðgang", "Créer un compte GitHub"),
    "GitHub's sign-in ended before it was finished. Press Sign in to try again.": _row("GitHub-inloggningen avbröts innan den var klar. Tryck på Logga in för att försöka igen.", "Die GitHub-Anmeldung wurde vor dem Abschluss beendet. Klicken Sie auf „Anmelden“, um es erneut zu versuchen.", "El inicio de sesión de GitHub terminó antes de completarse. Pulse Iniciar sesión para volver a intentarlo.", "GitHub 登录在完成前已结束。请点击“登录”重试。", "O login do GitHub terminou antes de ser concluído. Clique em Entrar para tentar novamente.", "GitHub का साइन-इन पूरा होने से पहले ही समाप्त हो गया। फिर से प्रयास करने के लिए साइन इन करें दबाएँ।", "GitHub 로그인이 완료되기 전에 종료되었습니다. 다시 시도하려면 로그인을 누르세요.", "Innskráningu hjá GitHub lauk áður en hún kláraðist. Ýttu á Skrá inn til að reyna aftur.", "La connexion à GitHub s’est arrêtée avant la fin. Cliquez sur Se connecter pour réessayer."),
    "Halcyon": _row("Halkyon", "Halkyon", "Alción", "宁静", "Alcíone", "शांत दिन", "평온", "Kyrrð", "Alcyon"),
    "Install": _row("Installera", "Installieren", "Instalar", "安装", "Instalar", "इंस्टॉल करें", "설치", "Setja upp", "Installer"),
    "Install runs:\n    {command}": _row("Installera kör:\n    {command}", "„Installieren“ führt aus:\n    {command}", "Instalar ejecuta:\n    {command}", "“安装”将运行：\n    {command}", "Instalar executa:\n    {command}", "इंस्टॉल करें यह चलाता है:\n    {command}", "설치 시 실행되는 명령:\n    {command}", "Setja upp keyrir:\n    {command}", "Installer exécute :\n    {command}"),
    "Install runs:\n    {command}\n\nThen GitHub's own sign-in starts.": _row("Installera kör:\n    {command}\n\nSedan startar GitHubs egen inloggning.", "„Installieren“ führt aus:\n    {command}\n\nDanach startet die Anmeldung von GitHub selbst.", "Instalar ejecuta:\n    {command}\n\nDespués se inicia el propio inicio de sesión de GitHub.", "“安装”将运行：\n    {command}\n\n随后启动 GitHub 自己的登录流程。", "Instalar executa:\n    {command}\n\nEm seguida, começa o login do próprio GitHub.", "इंस्टॉल करें यह चलाता है:\n    {command}\n\nइसके बाद GitHub का अपना साइन-इन शुरू होता है।", "설치 시 실행되는 명령:\n    {command}\n\n그런 다음 GitHub 자체 로그인이 시작됩니다.", "Setja upp keyrir:\n    {command}\n\nSíðan hefst innskráning GitHub sjálfs.", "Installer exécute :\n    {command}\n\nEnsuite, la connexion propre à GitHub démarre."),
    "Keep installing": _row("Fortsätt installera", "Installation fortsetzen", "Seguir instalando", "继续安装", "Continuar instalando", "इंस्टॉल करना जारी रखें", "계속 설치", "Halda uppsetningu áfram", "Poursuivre l’installation"),
    "Keep the screen open until it finishes, or stop it and install later from Help > Set spaCR up again…": _row("Låt skärmen vara öppen tills den är klar, eller stoppa den och installera senare via Hjälp > Konfigurera spaCR igen…", "Lassen Sie diesen Bildschirm geöffnet, bis sie abgeschlossen ist, oder brechen Sie sie ab und installieren Sie später über Hilfe > spaCR erneut einrichten…", "Mantenga la pantalla abierta hasta que termine, o deténgala e instale más tarde desde Ayuda > Configurar spaCR de nuevo…", "请保持此界面打开直到安装完成，或停止安装，稍后通过“帮助 > 重新设置 spaCR…”进行安装。", "Mantenha a tela aberta até terminar, ou interrompa e instale mais tarde em Ajuda > Configurar o spaCR novamente…", "इसके पूरा होने तक स्क्रीन खुली रखें, या इसे रोकें और बाद में सहायता > spaCR फिर से सेट अप करें… से इंस्टॉल करें।", "완료될 때까지 화면을 열어 두거나, 중지하고 나중에 도움말 > spaCR 다시 설정…에서 설치하세요.", "Hafðu skjáinn opinn þar til henni lýkur, eða stöðvaðu hana og settu upp síðar úr Hjálp > Setja spaCR upp aftur…", "Laissez l’écran ouvert jusqu’à la fin, ou arrêtez l’installation et installez plus tard depuis Aide > Reconfigurer spaCR…"),
    ...
    "Set up the GitHub CLI": _row("Konfigurera GitHub CLI", "GitHub CLI einrichten", "Configurar GitHub CLI", "设置 GitHub CLI", "Configurar o GitHub CLI", "GitHub CLI सेट अप करें", "GitHub CLI 설정", "Setja upp GitHub CLI", "Configurer GitHub CLI"),
    "Signing in to {label}…": _row("Loggar in på {label}…", "Anmeldung bei {label}…", "Iniciando sesión en {label}…", "正在登录 {label}…", "Entrando em {label}…", "{label} में साइन इन हो रहा है…", "{label}에 로그인하는 중…", "Skrái inn á {label}…", "Connexion à {label}…"),
    "Solstice": _row("Solstånd", "Sonnenwende", "Solsticio", "至日", "Solstício", "संक्रांति", "동지", "Sólstöður", "Solstice"),
    "Stop it and close": _row("Stoppa den och stäng", "Abbrechen und schließen", "Detenerla y cerrar", "停止并关闭", "Interromper e fechar", "रोकें और बंद करें", "중지하고 닫기", "Stöðva og loka", "L’arrêter et fermer"),
    ...
    "The GitHub CLI is not installed.\n\nWith it, spaCR files a report under your GitHub account without opening the browser. Without it, reports still open in your browser.": _row("GitHub CLI är inte installerat.\n\nMed det skickar spaCR in en rapport under ditt GitHub-konto utan att öppna webbläsaren. Utan det öppnas rapporterna fortfarande i webbläsaren.", "Die GitHub CLI ist nicht installiert.\n\nDamit reicht spaCR einen Bericht unter Ihrem GitHub-Konto ein, ohne den Browser zu öffnen. Ohne sie werden Berichte weiterhin im Browser geöffnet.", "GitHub CLI no está instalado.\n\nCon él, spaCR presenta un informe con su cuenta de GitHub sin abrir el navegador. Sin él, los informes se siguen abriendo en el navegador.", "未安装 GitHub CLI。\n\n安装后，spaCR 可直接以您的 GitHub 账户提交报告，无需打开浏览器。未安装时，报告仍会在浏览器中打开。", "O GitHub CLI não está instalado.\n\nCom ele, o spaCR registra um relatório na sua conta do GitHub sem abrir o navegador. Sem ele, os relatórios continuam abrindo no navegador.", "GitHub CLI इंस्टॉल नहीं है।\n\nइसके साथ spaCR ब्राउज़र खोले बिना आपके GitHub खाते से रिपोर्ट दर्ज करता है। इसके बिना भी रिपोर्ट आपके ब्राउज़र में खुलती हैं।", "GitHub CLI가 설치되어 있지 않습니다.\n\n설치하면 spaCR이 브라우저를 열지 않고 GitHub 계정으로 보고서를 제출합니다. 설치하지 않아도 보고서는 브라우저에서 열립니다.", "GitHub CLI er ekki uppsett.\n\nMeð því sendir spaCR skýrslu inn undir GitHub-aðganginum þínum án þess að opna vafrann. Án þess opnast skýrslur áfram í vafranum.", "GitHub CLI n’est pas installé.\n\nAvec lui, spaCR dépose un rapport sous votre compte GitHub sans ouvrir le navigateur. Sans lui, les rapports s’ouvrent toujours dans votre navigateur."),
    "The installer for {tools} is still running. Closing this screen now stops it.": _row("Installationsprogrammet för {tools} körs fortfarande. Om du stänger skärmen nu stoppas det.", "Das Installationsprogramm für {tools} läuft noch. Wenn Sie diesen Bildschirm jetzt schließen, wird es abgebrochen.", "El instalador de {tools} sigue en ejecución. Si cierra esta pantalla ahora, se detendrá.", "{tools} 的安装程序仍在运行。现在关闭此界面会将其停止。", "O instalador de {tools} ainda está em execução. Fechar esta tela agora o interrompe.", "{tools} का इंस्टॉलर अभी भी चल रहा है। अभी यह स्क्रीन बंद करने से वह रुक जाएगा।", "{tools} 설치 프로그램이 아직 실행 중입니다. 지금 이 화면을 닫으면 설치가 중지됩니다.", "Uppsetningarforritið fyrir {tools} er enn í gangi. Ef þú lokar þessum skjá núna stöðvast það.", "Le programme d’installation de {tools} est toujours en cours. Fermer cet écran maintenant l’arrête."),
    ...
    "spaCR cannot install it here by itself: it needs {needs}, and none was found. Install with:\n    {command}": _row("spaCR kan inte installera det här på egen hand: det kräver {needs}, och inget sådant hittades. Installera med:\n    {command}", "spaCR kann es hier nicht selbst installieren: Dafür wird {needs} benötigt, und nichts davon wurde gefunden. Installieren Sie es mit:\n    {command}", "spaCR no puede instalarlo aquí por sí solo: necesita {needs} y no se encontró ninguno. Instálelo con:\n    {command}", "spaCR 无法在此自行安装：它需要 {needs}，但未找到。请使用以下命令安装：\n    {command}", "O spaCR não consegue instalá-lo aqui sozinho: ele precisa de {needs}, e nenhum foi encontrado. Instale com:\n    {command}", "spaCR इसे यहाँ स्वयं इंस्टॉल नहीं कर सकता: इसके लिए {needs} आवश्यक है, और कोई नहीं मिला। इससे इंस्टॉल करें:\n    {command}", "spaCR이 여기서 직접 설치할 수 없습니다. {needs}이(가) 필요하지만 찾지 못했습니다. 다음 명령으로 설치하세요:\n    {command}", "spaCR getur ekki sett þetta upp hér sjálft: það þarf {needs} og ekkert fannst. Settu upp með:\n    {command}", "spaCR ne peut pas l’installer ici par lui-même : il lui faut {needs}, et rien n’a été trouvé. Installez-le avec :\n    {command}"),
    "spaCR cannot run this itself here: it needs {needs}, and none was found.": _row("spaCR kan inte köra detta själv här: det kräver {needs}, och inget sådant hittades.", "spaCR kann dies hier nicht selbst ausführen: Dafür wird {needs} benötigt, und nichts davon wurde gefunden.", "spaCR no puede ejecutar esto aquí por sí solo: necesita {needs} y no se encontró ninguno.", "spaCR 无法在此自行运行：它需要 {needs}，但未找到。", "O spaCR não consegue executar isto aqui sozinho: ele precisa de {needs}, e nenhum foi encontrado.", "spaCR इसे यहाँ स्वयं नहीं चला सकता: इसके लिए {needs} आवश्यक है, और कोई नहीं मिला।", "spaCR이 여기서 직접 실행할 수 없습니다. {needs}이(가) 필요하지만 찾지 못했습니다.", "spaCR getur ekki keyrt þetta sjálft hér: það þarf {needs} og ekkert fannst.", "spaCR ne peut pas exécuter cela ici par lui-même : il lui faut {needs}, et rien n’a été trouvé."),
    "{label} is not installed yet.\n\nInstall runs the command below on this computer: it downloads the vendor's own installer and runs it, then starts the sign-in. spaCR never sees your credentials.": _row("{label} är inte installerat ännu.\n\nInstallera kör kommandot nedan på den här datorn: det hämtar leverantörens eget installationsprogram och kör det, och startar sedan inloggningen. spaCR ser aldrig dina inloggningsuppgifter.", "{label} ist noch nicht installiert.\n\n„Installieren“ führt den folgenden Befehl auf diesem Computer aus: Er lädt das Installationsprogramm des Anbieters herunter, führt es aus und startet dann die Anmeldung. spaCR sieht Ihre Zugangsdaten nie.", "{label} aún no está instalado.\n\nInstalar ejecuta el siguiente comando en este equipo: descarga el instalador del propio proveedor, lo ejecuta y luego inicia el inicio de sesión. spaCR nunca ve sus credenciales.", "{label} 尚未安装。\n\n“安装”会在这台计算机上运行下方命令：下载并运行供应商自己的安装程序，然后开始登录。spaCR 永远不会看到您的凭据。", "{label} ainda não está instalado.\n\nInstalar executa o comando abaixo neste computador: baixa o instalador do próprio fornecedor, executa-o e depois inicia o login. O spaCR nunca vê suas credenciais.", "{label} अभी इंस्टॉल नहीं है।\n\nइंस्टॉल करें इस कंप्यूटर पर नीचे दिया गया कमांड चलाता है: यह विक्रेता का अपना इंस्टॉलर डाउनलोड करके चलाता है, फिर साइन-इन शुरू करता है। spaCR आपके क्रेडेंशियल कभी नहीं देखता।", "{label}이(가) 아직 설치되지 않았습니다.\n\n설치를 누르면 이 컴퓨터에서 아래 명령이 실행됩니다. 공급업체의 자체 설치 프로그램을 내려받아 실행한 다음 로그인을 시작합니다. spaCR은 사용자의 자격 증명을 절대 보지 않습니다.", "{label} er ekki uppsett enn.\n\nSetja upp keyrir skipunina hér fyrir neðan á þessari tölvu: hún sækir eigið uppsetningarforrit framleiðandans, keyrir það og hefur síðan innskráninguna. spaCR sér aldrei innskráningarupplýsingarnar þínar.", "{label} n’est pas encore installé.\n\nInstaller exécute la commande ci-dessous sur cet ordinateur : elle télécharge le programme d’installation propre à l’éditeur, l’exécute, puis lance la connexion. spaCR ne voit jamais vos identifiants."),
}
