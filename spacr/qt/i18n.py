...
_ROWS: Dict[str, tuple[str, ...]] = {
    ...
    "Primary modules are grouped here into Core, Data, Segmentation models, Results & QC, Explore, Assays and Design; related workflows are reached from their host module. Click any name to open it. Ctrl+1 through Ctrl+9 opens the first nine apps in sidebar order.": _row(
        "Primära moduler är grupperade här i Kärna, Data, Segmenteringsmodeller, Resultat och QC, Utforska, Analyser och Design; relaterade arbetsflöden nås från sin värdmodul. Klicka på ett namn för att öppna det. Ctrl+1 till Ctrl+9 öppnar de första nio apparna i sidofältets ordning.",
        "Die Hauptmodule sind hier in Kern, Daten, Segmentierungsmodelle, Ergebnisse und QC, Erkunden, Assays und Entwurf gruppiert; zugehörige Arbeitsabläufe erreichen Sie über ihr übergeordnetes Modul. Klicken Sie auf einen Namen, um ihn zu öffnen. Ctrl+1 bis Ctrl+9 öffnet die ersten neun Apps in der Reihenfolge der Seitenleiste.",
        "Los módulos principales se agrupan aquí en Principal, Datos, Modelos de segmentación, Resultados y CC, Explorar, Ensayos y Diseño; los flujos relacionados se abren desde su módulo anfitrión. Haga clic en un nombre para abrirlo. De Ctrl+1 a Ctrl+9 se abren las nueve primeras aplicaciones en el orden de la barra lateral.",
        "主要模块在这里分为核心、数据、分割模型、结果与质控、探索、实验分析和实验设计；相关流程可从其宿主模块进入。点击名称即可打开。Ctrl+1 至 Ctrl+9 按侧边栏顺序打开前九个应用。",
        "Os módulos principais estão agrupados aqui em Principal, Dados, Modelos de segmentação, Resultados e CQ, Explorar, Ensaios e Planejamento; os fluxos relacionados são acedidos a partir do respetivo módulo anfitrião. Clique num nome para o abrir. De Ctrl+1 a Ctrl+9 abrem as primeiras nove aplicações pela ordem da barra lateral.",
        "मुख्य मॉड्यूल यहाँ मुख्य, डेटा, खंडन मॉडल, परिणाम और QC, अन्वेषण, एसे और डिज़ाइन में समूहित हैं; संबंधित वर्कफ़्लो उनके होस्ट मॉड्यूल से खोले जाते हैं। किसी नाम पर क्लिक करके उसे खोलें। Ctrl+1 से Ctrl+9 साइडबार क्रम में पहले नौ ऐप खोलते हैं।",
        "주요 모듈은 핵심, 데이터, 분할 모델, 결과 및 QC, 탐색, 어세이 및 설계로 그룹화되어 있습니다. 관련 워크플로는 호스트 모듈에서 열 수 있습니다. 이름을 클릭하면 열립니다. Ctrl+1부터 Ctrl+9까지는 사이드바 순서대로 처음 아홉 개 앱을 엽니다.",
        "Aðaleiningar eru flokkaðar hér í Kjarna, Gögn, Hlutunarlíkön, Niðurstöður og gæðaeftirlit, Kanna, Prófanir og Hönnun; tengd vinnsluferli eru opnuð úr hýsingareiningunni. Smelltu á heiti til að opna það. Ctrl+1 til Ctrl+9 opnar fyrstu níu forritin í röð hliðarstikunnar.",
        "Les modules principaux sont regroupés ici dans Cœur, Données, Modèles de segmentation, Résultats et CQ, Explorer, Essais et Conception ; les flux associés sont accessibles depuis leur module hôte. Cliquez sur un nom pour l’ouvrir. De Ctrl+1 à Ctrl+9 ouvrent les neuf premières applications dans l’ordre de la barre latérale."),
