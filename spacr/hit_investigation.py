...
def register_settings(replace: bool = False) -> bool:
    ...
    tips = {
        ...
        "guide_fractions_file": "(str) - Sequencing-derived table with one fraction per well and guide. Duplicate well-guide rows are refused because they would reweight target evidence ambiguously. Default is an empty string.",
        "results_folder": "(str) - Exact regression output folder that produced the selected hit. Its CSV and JSON bytes are hashed so later edits create different provenance. Default is an empty string.",
        "target_gene": "(str) - Gene identifier carried from the selected Hit List row. It labels the attribution run and output folder without being inferred again. Default is an empty string.",
        ...
        "hit_phenotype": "(str) - Human-readable phenotype copied from the source regression. It records what direction and score the selected effect actually described. Default is an empty string.",
        ...
        "hit_guide_agreement": "(float) - Fraction of the target gene's tested guides agreeing with the selected effect direction. It is copied from the hit result for provenance and display. Default NaN when unavailable.",
        "hit_n_guides": "(int) - Number of tested guides supporting the selected gene in the source regression family. It is provenance rather than a cell-level measurement. Default 0.",
        "hit_well_support": "(int) - Number of independent source wells supporting the selected hit across its guides. It is copied for provenance and does not weight candidate cells. Default 0.",
