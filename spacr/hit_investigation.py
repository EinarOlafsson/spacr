...
def register_settings(replace: bool = False) -> bool:
    ...
    tips = {
        ...
        "guide_fractions_file": "(str) - Sequencing-derived table with one fraction per well and guide. Duplicate well-guide rows are refused because they would reweight target evidence ambiguously. Default blank; select a file before running.",
        "results_folder": "(str) - Exact regression output folder that produced the selected hit. Its CSV and JSON bytes are hashed so later edits create different provenance. Default blank; select a folder before running.",
        "target_gene": "(str) - Gene identifier carried from the selected Hit List row. It labels the attribution run and output folder without being inferred again. Default blank until a hit is selected.",
        ...
        "hit_phenotype": "(str) - Human-readable phenotype copied from the source regression. It records what direction and score the selected effect actually described. Default blank until a hit is selected.",
        "hit_effect": "(float) - Effect estimate copied from the exact source result for provenance and display. It does not get re-estimated from candidate cells. Default 0.0 until a hit is selected.",
        "hit_fdr": "(float) - Adjusted P value copied from the selected regression hit. It records source evidence and never becomes a cell-level confidence value. Default 1.0 until a hit is selected.",
        "hit_guide_agreement": "(float) - Fraction of the selected gene's guides whose effects agree with its reported direction. It is provenance, not a cell-level probability. Default NaN until a hit is selected.",
        "hit_n_guides": "(int) - Number of distinct guides supporting the selected regression hit. It is carried into the attribution manifest without being recomputed. Default 0 until a hit is selected.",
        "hit_well_support": "(int) - Number of independent wells supporting the selected regression hit. It records source evidence and does not change candidate-cell fitting. Default 0 until a hit is selected.",
        ...
        "hit_feature_columns": "(list) - Independent measured morphology features used by the weak-supervision model. Leave blank for guarded numeric selection excluding identifiers, labels, fractions and scores. Default [].",
