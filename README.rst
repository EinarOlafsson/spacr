...
The figures above are the ones measured at publication, and the limits are
stated with them: a model is useful for the job it was measured on, not for
every job. ``toxoplasma_well_detector_v1`` and ``toxoplasma_plaque_v1`` are the
two halves of one pipeline -- the detector finds the wells, the segmenter finds
the plaques inside them, and the well diameter is what makes areas comparable
between microscopes.
